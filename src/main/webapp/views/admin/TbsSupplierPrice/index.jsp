			...
			<div class="page-content">
				...
				<div class="widget-box transparent">
					<div class="widget-header widget-header-flat">
						<h4 class="lighter">
							<i class="fa fa-list-ol orange"></i> 供应商单价列表
						</h4>
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		$('#icon-bureau-add').on( 'click', function(e) {
			e.preventDefault();
			var title = "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-plus-square red'></i> 新增供应商单价信息</h4></div>";
			T.Box.WSC_ShowDialog("admin/supplierPrice/baseDlg", 400, title);
	    	T.C.S.Validform.resetForm();
	    	$("#obj_id").val('');
	    	T.U.ajaxUtil('admin/supplier/comboxList',"#obj_SupplierId");
        	return false;
		 ...
		 $('#icon-bureau-edit').on( 'click', function(e) {
			e.preventDefault();
			var title = "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon fa fa-pencil red'></i> 修改供应商单价信息 </h4></div>";
			...
			if(T.C.S.i == 1){
	    	T.C.S.Validform.resetForm();
	    	T.U.ajaxUtil('admin/supplier/comboxList',"#obj_SupplierId",false);
	    	T.C.S.SKEY = rowData.id;
			$("#obj_id").val(rowData.id);
			$("#obj_SupplierId").val(rowData.supplierId);
			$("#obj_Price").val(rowData.prices);
		 ...
		 $('#icon-bureau-delete').on( 'click', function(e) {
			...
			var rowData = jQuery(grid_selector).jqGrid('getRowData',rowId);
			T.C.S.SKEY = rowData.id;
			T.C.S.PRICE = rowData.supplierId;
		...
		$("body").on( 'click',"#btn-ico-confirmation", function(e) {
			e.preventDefault();
			T.U.AjaxRequest({
   					url : T.U.RewriteAjaxAction('admin/supplierPrice/del/'+T.C.S.SKEY),type:"POST",
   					params:{supplierId:T.C.S.PRICE},
   					callback : function(data) {
   						   if (typeof (data)  != 'undefined') {
		                    	if(data.success){
		                    		jQuery(T.C.S.DridSelector).jqGrid('setGridParam', {
		            					url : T.U.RewriteAjaxAction('admin/supplierPrice/data'+"?_t=" + new Date().getTime()),
		            					mtype:'POST'
		            				}).trigger("reloadGrid");
		...
		var _colNames = ['ID','公司ID', '公司名称', '单价num','单价（元/斤）', '备注', '创建时间'];
		...
		var _colModel = [ {name : 'id', index : 'id', width : 60, sorttype : "text", hidden : true,key : true }, 
						  ...
						  {name : 'name',index : 'name',width : 90,editable : true,sorttype : "text"}, 
						   {name : 'prices',index : 'prices',width : 60,hidden : true},
						  {name : 'price',index : 'price',width : 60,formatter:customFmatter}, 
						  {name : 'remarks', index : 'remarks', width : 150,sorttype : "text"  },
						  {name : 'createTime',index : 'createTime',width : 100,sorttype : "text"} ];
						  
		T.D._initDataGrid(grid_selector,pager_selector,"admin/supplierPrice/data",_colNames,_colModel);
		
		function customFmatter(cellvalue, options, rowObject){  
      		
      				if(cellvalue < 5){
      					return '<span class="label label-sm label-success arrowed arrowed-righ" style="width:150px;">'+cellvalue+'</span>';
      				}else if(cellvalue < 6){
      					return '<span class="label label-sm label-success arrowed arrowed-righ" style="width:150px;">'+cellvalue+'</span>';
      				}else if(cellvalue < 7){
      					return '<span class="label label-sm label-warning arrowed arrowed-righ" style="width:70px;">'+cellvalue+'</span>';
      				}else{
      					return '<span class="label label-sm label-danger arrowed-in" style="width:150px;">'+cellvalue+'</span>';
      				}
		}  
	});
</script>
