	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true || ts.p.treeGrid === true) {
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading || ts.p.treeGrid === true) {
				...
				case "script":
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							...
							if( ts.p.loadonce ) {ts.p.datatype = "json";}
							data=null;
							if (npage === 1) { endReq(); }
						},
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0 || ts.p.treeGrid === true){
				str = "<td dir='"+dir+"'>";
			...
			if(dir==="rtl") { pgl += str; }
			if(ts.p.pginput===true || ts.p.treeGrid === true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true || ts.p.treeGrid === true) {
...
$.jgrid.extend({
	setTreeNode : function(i, len){
		return this.each(function(){
			...
			while(i<len) {
				...
				twrap += "<div style='"+($t.p.direction==="rtl" ? "right:" : "left:")+(lftpos*18)+"px;' class='ui-icon fa ";
	...
	setTreeGrid : function() {
		return this.each(function (){
			 ...
			 * 用于TreeGrid 分页
			 */
			if($t.p.rowTotal === null ) { $t.p.rowNum = 10; }
			//if($t.p.rowTotal === null ) { $t.p.rowNum = 10000; }
			$t.p.multiselect = false;$t.p.rowList = [10,20,30];
			$t.p.expColInd = 0;
